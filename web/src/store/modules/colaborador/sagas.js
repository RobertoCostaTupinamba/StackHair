...
export function* addColaborador() {
  try {
    const { colaborador, form, components, behavior } = yield select((state) => state.colaborador);
...
export function* unlinkColaborador() {
  const { form, components, colaborador } = yield select((state) => state.colaborador);
...
export function* saveColaborador() {
  ...
  try {
    ...
    const { data: res } = yield call(api.put, `/colaborador/${colaborador._id}`, {
      vinculo,
      vinculoId,
      especialidades,
    });
